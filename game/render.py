...
import pygame
from game.core import GameState, Vec3, angle_between, cross, dot, normalize, scale, sub
...
def _grid_points(step_key: int) -> List[Vec3]:
    step = step_key / 1000.0
    step = max(0.02, min(0.5, step))
    pts: List[Vec3] = []
    lat = -math.pi / 2
    while lat <= math.pi / 2 + 1e-6:
        cos_lat = math.cos(lat)
        sin_lat = math.sin(lat)
        lon = 0.0
        while lon < 2 * math.pi - 1e-6:
...
def draw_frame(surface: pygame.Surface, state: GameState) -> None:
    ...
    if len(state.snake) > 1:
        seg_angle = angle_between(state.snake[0], state.snake[1])
    else:
        seg_angle = 0.045
    step_key = max(1, int(round(seg_angle * 1000)))
    grid_points = _grid_points(step_key)
